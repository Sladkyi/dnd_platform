...
const Profile = ({ profile }) => {
  ...
  const handleEditMap = (mapId) => {
    navigate(`/edit-map/${mapId}`, {
      state: {
        role: 'master', // 👈 передаём явно
      },
    });
  };

  const createGameRoom = () => {
    navigate(`/create-room/${profile.id}`, {
      state: {
        maps: profile.maps,
      },
    });
    ...
    <div className="profile-container">
      ...
      <main className="main-content">
        ...
        <section className="actions-section">
          <button onClick={() => handleGoToMaps()} className="primary-button">
            + Создать карту
          </button>
          <button onClick={() => handleJoinGame()} className="primary-button">
