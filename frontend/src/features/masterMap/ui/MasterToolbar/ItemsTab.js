...
const ItemsTab = () => {
  ...
  const handleDragStart = (e, item) => {
    ...
    e.dataTransfer.setData(
      'application/json',
      JSON.stringify({ type: 'item', id: item.id })
    );
            ...
            {hoveredItem === item.id && (
              <div className="item-tooltip">
                ...
                <div className="tooltip-content">
                  ...
                  <p><strong>Цена:</strong> {item.value} з.</p>
                  {item.properties && <p><strong>Свойства:</strong> {item.properties}</p>}
                  {item.description && <p className="tooltip-description">{item.description}</p>}
                </div>
              </div>
            )}
          </div>
